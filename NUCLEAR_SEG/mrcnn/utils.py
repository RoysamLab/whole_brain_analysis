...
def compute_matches(gt_boxes, gt_class_ids, gt_masks,
    ...
    match_count = 0
    pred_match = -1 * np.ones([pred_boxes.shape[0]])  # -1 for negative 
    gt_match   = -1 * np.ones([gt_boxes.shape[0]])    # -1 for negative 
...
def compute_F1_range(gt_box, gt_class_id, gt_mask,
                     pred_box, pred_class_id, pred_score, pred_mask,
                     iou_thresholds=None, verbose=0):
    """Compute AP over a range or IoU thresholds. Default range is 0.5-0.95."""
    # Default is 0.5 to 0.95 with increments of 0.05
    ...
    for iou_threshold in iou_thresholds:
        ap, precisions, recalls, overlaps =\
            compute_ap(gt_box, gt_class_id, gt_mask,
                        pred_box, pred_class_id, pred_score, pred_mask,
                        iou_threshold = iou_threshold)
        f1scores = 2 * recalls * precisions / (recalls + precisions)
        if verbose:
            print("F1 @{:.2f}:\t {:.3f}".format(iou_threshold, f1scores))
        
        F1.append(f1scores.mean())
    # F1 = np.array(f1scores).mean()
    # if verbose:
    #     print("F1 @{:.2f}-{:.2f}:\t {:.3f}".format(
    #         iou_thresholds[0], iou_thresholds[-1], AP))
    return F1
