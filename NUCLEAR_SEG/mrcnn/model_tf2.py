...
OrignialWritten by Waleed Abdulla

Credit of Convert to tf2
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    negative_indices = tf.compat.v1.where(tf.logical_and(roi_iou_max < config.ROI_IOU_thres, no_crowd_bool))[:, 0]
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    def nms_keep_map(class_id):
        ...
        class_keep = tf.image.non_max_suppression(
                ...
                iou_threshold=float(config.DETECTION_NMS_THRESHOLD))
        # Map indices
...
class MaskRCNN(object):
    ...
    def __init__(self, mode, config, model_dir,retrain= False):
        """
        ...
        self.retrain = retrain                          # [debug retrain to result log]
    ...
    def build(self, mode, config):
        ...
        if mode == "training" or mode== "retrain":
            # RPN GT
        ...
        if model_path:
            ...
            if self.retrain == True:
                ...
                if m:
                    ...
                    self.epoch = int(m.group(6)) - 1 + 1     # [debug, retrain to start from epoch0]
            # import pdb;pdb.set_trace()
    ...
    def detect(self, images, verbose=0, intermediate_layers = []):
        ...
        #####################Save Intermediate Results                      #edit
        if len(intermediate_layers) >0 :
            model  = self.keras_model        
            intermediate_output = {}
            print ("\n intermediate_layers =", intermediate_layers)
            for layer in self.keras_model.layers:
                # save configs into xml
                
                # save layeroutput as featurepath into numpy array
                if layer.name in intermediate_layers:
                    print ("...intermediate_layer=",layer.name)
                    intermediate_layer_model = KM.Model ( model.input, layer.output )                            # https://stackoverflow.com/questions/41711190/keras-how-to-get-the-output-of-each-layer
                    intermediate_output[layer.name] = intermediate_layer_model.predict([molded_images, image_metas, anchors])
            #####################Save Intermediate Results                      #edit
        ...
        for i, image in enumerate(images):
            ...
            if len(intermediate_layers) >0 :
                for layerName in intermediate_layers:
                    result_dict[layerName] = intermediate_output[layerName]
