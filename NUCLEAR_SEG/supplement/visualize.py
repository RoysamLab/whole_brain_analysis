    ...
    for i in range(N):
        ...
        if show_mask:
            if mask_erosion_px is not None:
                mask = skimage.morphology.binary_erosion(mask, skimage.morphology.disk(mask_erosion_px))   # make mask thicker
...
def draw_boxes(image, boxes=None, refined_boxes=None,
    ...
    for i in range(N):
        ...
        if boxes is not None:
            ...
            y1, x1, y2, x2 = boxes[i]
            if seeds == True:
                x1,x2= (x1+x2)//2 ,(x1+x2)//2+1
                y1,y2= (y1+y2)//2 ,(y1+y2)//2+1
            p = patches.Rectangle((x1, y1), x2 - x1, y2 - y1, linewidth=2,
