...
#plt.title('[Histone + DAPI] filled image ')
    ...
    local_thres = filters.threshold_local(img,37,method = 'gaussian')
    
    bin_mask_level1 = binMaskCorrection(img, (0.9 - offset ) * otsu_thres)
#    plt.imshow(img,cmap ='gray')
    img = img *  ( (img> 0.6 *otsu_thres) + (img> local_thres))
#    bin_mask_level1 = img[img>otsu_thres]>local_thres
