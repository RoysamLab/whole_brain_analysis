...
#    bin_mask = morphology.binary_opening (bin_mask,morphology.disk(3))                 # remove white noise for
    ...
    bin_mask = ndimage.binary_fill_holes(bin_mask, morphology.disk(4))                  # filling holes
...
#    fill_in_pixced_smoothed = fill_in_pixcel[fill_in_pixcel>0].mean() + filters.gaussian_filter(fill_in_pixcel, sigma=2)
 ...
 # #################                      seconde round fill in   # for those hole are too big
    ...
    filledCell = imSummation(img_fl, 0.6*fill_in_pixced_smoothed, outputformat = 'float')
...
#        plt.figure(),plt.imshow(label_level1) 
    ...
    #            connected_obj = PropertyTable_1st[209]
                ...
                bin_mask_border       = binMaskCorrection(connected_Crop_enlarged, otsu_thres) 
#                
                if maskCorrectR!=0:   # fill in holes
...
#                D = 5*D_exactShape + D_orignal *(D_exactShape==0)
